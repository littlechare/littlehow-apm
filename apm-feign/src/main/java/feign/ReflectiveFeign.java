...
/**
 * ReflectiveFeign本来在feign核心包中，但是需要对其进行扩展，所以将资源类拿来修改，然后生成二进制进行优先加载
 * 实际资源以及管理到
 * @see com.littlehow.apm.feign.ReflectiveFeignSource 中
 * @author littlehow
 */
public class ReflectiveFeign extends Feign {
    ...
    private static class BuildTemplateByResolvingArgs implements RequestTemplate.Factory {
        ...
        public RequestTemplate create(Object[] argv) {
            ...
            String url = mutable.url();
            if (url.contains("{") && url.contains("}")) {
                //提前解析参数
                AdviceExecutor.setPreInfo(url, argv);
            }
